...
public class SmaliClassDetailLoader extends ClassDetailLoader {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SmaliClassDetailLoader.class);
    ...
    public static SmaliClassDetailLoader fromApkFile(ZipFile apkFile, int apiLevel, boolean translateInstructions) {
        ...
        if (n == 0) {
            logger.error("Source apk does not have any dex files");
        }
        ...
        try {
            ...
            logger.error("failed to process the source apk file");
    ...
    public static SmaliClassDetailLoader fromFramework(File frameworkClassesFolder, int apiLevel) {
        ...
        DexFile dex;
        try {
            dex = DexFileFactory.loadDexFile(f, Opcodes.forApi(apiLevel));
        } catch (IOException e) {
    ...
    public void loadAll(Scope scope) {
        ...
        for (DexFile dexFile : dexFiles) {
            // 提取dex中的class和method
            parseDex(dexFile, scope, collector);
        }

        if (translateInstructions) {
            // 逐句解析出method内容
            parseMethod(scope, collector);

        }

    }

    // 多个dex时，会导致OutOfMemory，改为每个dex分别分析
    public void loadDex() {
        IdentityHashMap<MethodInfo, MethodImplementation> collector = new IdentityHashMap<MethodInfo, MethodImplementation>();
        for (DexFile dexFile : dexFiles) {
    ...
    private void parseDex(DexFile dexFile, Scope scope, IdentityHashMap<MethodInfo, MethodImplementation> collector) {
        for (final ClassDef classDef : dexFile.getClasses()) {
            String classType = classDef.getType();
            if ((classType.startsWith("Landroid/")) || (classType.startsWith("Landroidx/"))) {
                continue;
            }
    ...
    private void parseMethod(Scope scope, IdentityHashMap<MethodInfo, MethodImplementation> collector) {
...
//        logger.info("Translating method: %s", mi.toString());
